...
void LayerManager::setPerfMode(PerfMode mode)
{
    ...
    JT_CC_LOG("[LayerMgr] PerfMode → %s  voices A=%u B=%u\n",
...
void LayerManager::setVoiceSplit(uint8_t voicesA)
{
    ...
    JT_CC_LOG("[LayerMgr] VoiceSplit → A=%u B=%u\n",
...
void LayerManager::setSplitNote(uint8_t note)
{
    ...
    JT_CC_LOG("[LayerMgr] SplitNote → %u\n", (unsigned)_splitNote);
}
...
{
    _editTarget = t;
    JT_CC_LOG("[LayerMgr] EditTarget → %s\n",
...
void LayerManager::setMidiChannelA(uint8_t ch1to16)
{
    ...
    _midiChannelA = newCh;
    JT_CC_LOG("[LayerMgr] MIDI Ch A → %u\n", (unsigned)_midiChannelA);
}
...
void LayerManager::setMidiChannelB(uint8_t ch1to16)
{
    ...
    _midiChannelB = newCh;
    JT_CC_LOG("[LayerMgr] MIDI Ch B → %u\n", (unsigned)_midiChannelB);
}
...
void LayerManager::setCC(uint8_t cc, uint8_t value) {
    if (_isPerfCC(cc)) {
        // Perf CCs go through handleControlChange which fires the notifier
        // internally — no double-fire needed.
        ...
        // Fire notifier — enables TFT repaint (ui.notifyCC) and SysEx
        // PARAM_VALUE echo (notifyLocalCC) for patch-scope CCs.
        // Perf CCs skip this because handleControlChange fires it already.
        if (_notifyFn) _notifyFn(cc, value);
    }
}
