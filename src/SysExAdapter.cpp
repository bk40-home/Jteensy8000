...
void SysExAdapter::_handleSetParam(uint8_t layer, uint16_t paramId, float value) {
    ...
    // Convert SysEx float -> CC byte using the param's value type.
    const uint8_t ccValue = _floatToCC(e->type, value);
...
float SysExAdapter::_ccToFloat(uint8_t valueType, uint8_t ccValue) const {
    using namespace ParamMap;

    switch (valueType) {
        case kCont:
            // Editor receives the CC byte as-is (as float). Phase 3 will
            // add a true native-float read path.
            return (float)ccValue;

        case kBipolar:
            // Inverse of _floatToCC's two-segment encode: dead-zone at 64.
            if (ccValue == 64) return 0.0f;
            if (ccValue >  64) return ((float)ccValue - 64.0f) / 63.0f; // 0+..1
            return ((float)ccValue - 64.0f) / 64.0f;                    // -1..0-

        case kNorm01:
            return (float)ccValue / 127.0f;

        case kEnumN:
            // Editor reads the raw CC value back as float (since we don't
            // know N here). Editor decodes per-param.
            return (float)ccValue;

        case kToggle:
            // Post-Q4 standardisation: threshold >=64.
        ...
        case kSignedOff:
            // CC 0 -> -1.0 (OFF/preset); CC 1..127 -> 0..126 (raw, editor decodes).
            if (ccValue == 0) return -1.0f;
            return (float)(ccValue - 1);
...
//
// Each value type has its own conversion. Designed to round-trip cleanly with
// the firmware's CC handlers (see Mapping.h / SynthEngine.cpp for the inverse).
// =============================================================================
uint8_t SysExAdapter::_floatToCC(uint8_t valueType, float value) const {
    ...
    switch (valueType) {
        case kCont:
            // For continuous params the editor sends the CC value directly
            // (0..127 as a float). The firmware then re-applies the curve.
            // Phase 3 will introduce float-direct setters that bypass this.
            return clampToCC(value);

        case kBipolar:
            // Editor sends -1.0..+1.0; we map to CC 0..127 with CC 64 = zero.
            // Symmetric mapping mirroring the Q1/Q2 firmware fixes.
            if (value >= 0.0f) {
                if (value > 1.0f) value = 1.0f;
                return (uint8_t)(64.0f + value * 63.0f + 0.5f);   // 64..127
            } else {
                if (value < -1.0f) value = -1.0f;
                return (uint8_t)(64.0f + value * 64.0f + 0.5f);   // 0..63
            }

        case kNorm01:
            // Editor sends 0.0..1.0; clamp and scale to 0..127.
            if (value <= 0.0f) return 0;
            if (value >= 1.0f) return 127;
            return (uint8_t)(value * 127.0f + 0.5f);

        case kEnumN:
            // Editor sends the variation index as float (0.0, 1.0, 2.0...).
            // Firmware's CC handlers use bucket-midpoint decode. We don't
            // know N here without a wider lookup, so we use the simplest
            // safe form: pass the integer through scaled to CC space.
            // The firmware handler clamps anything out of range.
            return clampToCC(value);

        case kToggle:
            // Editor sends 0.0 / 1.0. Post-Q4 fix the firmware threshold is
            // >=64, so we emit 0 or 127 unambiguously.
            return (value >= 0.5f) ? 127 : 0;

        case kInt7:
            // Raw 0..127 byte, sent as float.
            return clampToCC(value);

        case kSignedOff:
            // -1.0 = OFF (CC 0); 0.0..N as float = CC 1..127. The editor sends
            // -1.0 explicitly; otherwise we clamp to >=0 and offset by 1.
            if (value < 0.0f) return 0;
            return clampToCC(value + 1.0f);   // shift up so 0 -> CC 1

        case kSysEx:
        default:
            // Should not reach here for routed CCs. Defensive zero.
            return 0;
    }
}
...
void SysExAdapter::_applyEngineCC(uint8_t layer, uint8_t cc, uint8_t ccValue) {
    switch (layer) {
        case SyxProto::kLayerA:
            ...
            _lm.layerA().handleControlChange(1, cc, ccValue);
            break;
        case SyxProto::kLayerB:
            _lm.cacheCC(SyxProto::kLayerB, cc, ccValue);
            _lm.layerB().handleControlChange(1, cc, ccValue);
            break;
        case SyxProto::kLayerBoth:
            ...
            _lm.layerB().handleControlChange(1, cc, ccValue);
            break;
        default: break;   // already validated upstream
    }
}
