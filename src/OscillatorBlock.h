...
public:
    ...
    /**
     * @brief Pre-seed the glide start frequency before noteOn().
     *
     * In poly mode, a newly allocated voice may have a stale _baseFreq
     * from its previous note (or the 440 Hz default).  Calling this
     * before noteOn() ensures glide starts from the last note the
     * *synth* played, not the last note this particular voice played.
     *
     * Safe to call at any time — only affects the next glide start.
     */
    void setGlideFromFreq(float hz);
