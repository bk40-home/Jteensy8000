...
void OscillatorBlock::setGlideFromFreq(float hz) {
    // Pre-seed the base frequency so the next noteOn() glide starts
    // from this pitch rather than whatever stale value _baseFreq holds.
    // Only meaningful when glide is enabled — harmless otherwise.
    if (hz > 20.0f) _baseFreq = hz;
}
