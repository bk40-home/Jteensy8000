...
// =============================================================================
// The hot path (tick / tickBlock) lives in the header so it inlines. This file
// holds the rarely-called setters that recompute the cached coefficients.
// =============================================================================
...
{
    // 1 µs floor — effectively "snap immediately" without a divide-by-zero in
...
{
    _target = t;
    // If we are already at the target (within EPS), stay settled — avoids
    // burning CPU on a slew with nowhere to go. Otherwise wake up.
...
void SlewedValue::recomputeG()
{
    ...
    // g via the bank's standard pre-warp — the same function the cores use.
